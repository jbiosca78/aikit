...
function cardTemplate(item) {
    ...
    const imageSrc = `img/${item.modelo}.jpg`;
    const modelUrl = `${item.modelo}.html`;

    return `
        <article class="card">
            <img src="${imageSrc}" alt="${item.nombre}" onerror="this.src='img/placeholder-armario.svg'" />
            <div class="card-body">
                <div class="badges">
...
async function initCatalog() {
    ...
    try {
        ...
        function render() {
            ...
            if (!filtered.length) {
                root.innerHTML = '<div class="empty">No hay armarios con esos filtros. Prueba a ampliar ancho/fondo de habitacion o reducir carga por balda.</div>';
                return;
            }
        ...
        root.innerHTML = `<div class="empty">Error cargando catalogo: ${err.message}</div>`;
    }
}
